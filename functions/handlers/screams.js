...
const checklikes = async (userHandle) => {
    try {
       return await db.collection("likes")
            .where('userHandle', '==', userHandle)
            .get()
            .then((data) => {
                let likedScreamsByuser;
                likedScreamsByuser = [];
                data.forEach((doc) => {
                    likedScreamsByuser.push(doc.data().screamId)
                })
                return likedScreamsByuser;
            })
    } catch (err) {
        console.error(err)
    }
...
const checkdislikes = async (userHandle) => {
    try {
        return await db.collection("unlikes")
            .where('userHandle', '==', userHandle)
            .get()
            .then((data) => {
                let dislikedScreamsByuser;
                dislikedScreamsByuser = [];
                data.forEach((doc) => {
                    dislikedScreamsByuser.push(doc.data().screamId)
                })
                return dislikedScreamsByuser;
            })
    } catch (err) {
        console.error(err)
    }
...
exports.getAllScreams = async (req, res) => {
    try {
        const likedScreamByUSer = await checklikes(req.user.handle);
        const dislikedScreamByUser = await  checkdislikes(req.user.handle);
        const data = await db
            .collection("screams")
        ...
        data.forEach((doc) => {
            screams.push({
                ...
                isLiked:likedScreamByUSer.includes(doc.id),
                isDisliked:dislikedScreamByUser.includes(doc.id)
            });
        });
